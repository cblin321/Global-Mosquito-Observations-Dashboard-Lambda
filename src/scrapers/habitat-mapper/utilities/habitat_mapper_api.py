################################################################################
#                                                                              #
#                            habitat_mapper_api.py                             #
#                                                                              #
################################################################################
#                                                                              #
#        This is a script for interfacing with the Habitat Mapper API.         #
#                                                                              #
#        Author(s): Abe Megahed                                                #
#                                                                              #
#        This file is subject to the terms and conditions defined in           #
#        'LICENSE.txt', which is part of this source code distribution.        #
#                                                                              #
################################################################################
#        Copyright (C) 2025 - Global Mosquito Observations Dashboard           #
################################################################################

import requests
import json
import sys
import math

sys.path.insert(0, '../../translators/habitat-mapper') 
from utilities.json_translator import HabitatMapperTranslator

class HabitatMapperAPI:

	#
	# static attributes
	#

	api = 'https://api.globe.gov/search/v1/measurement/protocol/measureddate/'
	params = {
		'protocols': 'mosquito_habitat_mapper',
		'geojson': 'FALSE',
		'sample': 'FALSE'
	}

	#
	# getting methods
	#

	@staticmethod
	def get_url(url, params):

		"""
		Get a url from a base url and a set of query string parameters.

		Args:
			url (string): The base url.
			params (dict): The collection of query string parameters.

		Returns:
			string: The url.
		"""

		query_string = ''
		for param in params:
			value = params[param]
			if value != None:
				query_string += ('?' if query_string == '' else '&') + param + '=' + str(value)
		return url + query_string

	#
	# counting methods
	#

	@staticmethod
	def num_observations(start_date, end_date):

		"""
		Fetch number of observations within a date range.

		Args:
			api: The api to fetch data from
			start_date: The start of the date range.
			end_date: The end of the date range.

		Returns:
			integer: The number of observations.
		"""

		# make request
		#
		response = requests.get(HabitatMapperAPI.get_url(HabitatMapperAPI.api, HabitatMapperAPI.params | {
			'startdate': start_date,
			'enddate': end_date
		}))

		# check for a response
		#
		if response is None:
			print("No results returned from API.")
			return 0

		# parse response
		#
		json_data = json.loads(response.text)
		return json_data['count'] if json_data else 0

	#
	# fetching methods
	#

	@staticmethod
	def fetch_data(start_date, end_date):

		"""
		Fetch data within a date range.

		Args:
			api: The api to fetch data from
			start_date: The start of the date range.
			end_date: The end of the date range.

		Returns:
			array: The data fetched from the api.
		"""

		# make request
		#
		response = requests.get(HabitatMapperAPI.get_url(HabitatMapperAPI.api, HabitatMapperAPI.params | {
			'startdate': start_date,
			'enddate': end_date
		}))

		# parse response
		#
		results = json.loads(response.text)

		# check for a response
		#
		if response is None:
			print("No results returned from API.")
			return []

		# parse observations
		#
		observations = results['results'] if 'results' in results else []

		# transform data
		#
		return HabitatMapperTranslator().translate_observations(observations)